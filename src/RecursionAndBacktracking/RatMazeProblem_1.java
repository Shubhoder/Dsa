...
The directions in which the rat can move are 'U'(up), 'D'(down), 'L' (left), 'R' (right). Value 0
at a cell in the matrix represents that it is blocked and rat cannot move to it while value 1 at a
...
public class RatMazeProblem_1 {
    public static ArrayList<String> findPath(int[][] m, int n) {
        // Your code here
        ArrayList<String> ans = new ArrayList<>();
        allPath(0, 0, n-1, n-1,"",ans,m);
        return ans;
    }
    private static void allPath(int sr , int sc , int er , int ec , String path ,ArrayList<String> ans, int[][] m){
        if(sr < 0 || sc < 0 || sr > er || sc > ec || m[sr][sc] == 0 ) return;
        if(sr==er && sc==ec){
            ans.add(path);
            return;
        }
        ...
        m[sr][sc]=0;
        //Down
        allPath(sr+1 , sc, er, ec ,path+"D",ans,m);
        //Left
        allPath(sr , sc-1, er, ec ,path+"L",ans,m);
        //Right
        allPath(sr , sc+1, er, ec ,path+"R",ans,m);
        //Up
        allPath(sr-1 , sc, er, ec ,path+"U",ans,m);
